...
// styles.typ. Must NOT import anything from templates/bespoke/ --
// its lib/ (styles.typ, typography.typ, entry.typ, talk.typ,
// grant.typ, publication.typ) encodes the private bespoke look
...
// Build the inline-markup helper for a given `meta`. Folds in the
// self-bold logic: normalize meta.self_bold into a tuple of terms
// (string, list, or missing -> always a tuple), then return an `mk`
// closure. YAML strings -> parsed Typst content; every self_bold term
// is wrapped in `*...*` before evaluating so the author's own name
// renders bold wherever it appears (author lists, presentation titles,
// etc.) without manual markup in every row.
#let make-mk(meta) = {
  let raw = meta.at("self_bold", default: ())
  let self-bold-terms = if type(raw) == str {
    if raw == "" { () } else { (raw,) }
  } else {
    raw
  }
...
// VERBATIM from scripts/cv_editor/author_flags.py:AUTHOR_FLAGS.
// ============================================================================
...
#let render-pub-body(e, mk) = {
  let af = format-authors(e.authors, mk)
  let body = af.content
  body += [. ]
...
#let render-appointments(data, mk) = {
  for sub in data {
...
#let render-publications(data, mk) = {
  for sub in data {
    let sub-audiences = sub.at("audiences", default: ())
    let sub-hide-from = sub.at("hide-from", default: ())
    ...
    for (i, e) in entries.enumerate() {
      row(marker: str(total - i) + ".", above: 9pt, render-pub-body(e, mk))
    }
  }
}
...
#let render-presentations(data, mk) = {
  for sub in data {
    let sub-audiences = sub.at("audiences", default: ())
    let sub-hide-from = sub.at("hide-from", default: ())
    let entries = sub.entries.filter(e => entry-visible(
    ...
    for (i, e) in entries.enumerate() {
      let notes = e.at("notes", default: ())
        .map(n => format-generic-note(n, mk)).filter(n => n != none)
...
#let render-research-support(data, mk) = {
...
// ============================================================================
// Per-section dispatch: take already-loaded `data`, skip (heading
// included) if nothing visible, else render the heading + body.
// `before` lets template.typ apply the first-section 30pt-before
// override. All 9 sections share this shape (data, title, any-visible
// predicate, body renderer, mk), so one small dispatcher replaces 9
// near-identical wrapper bodies. `data` is injected by the caller
// (template.typ's render loop); `mk` is threaded through to the body
// renderer.
// ============================================================================

#let _dispatch-section(data, title, any-visible, render-fn, mk, before: 24pt) = {
  if not any-visible(data) { return }
  section(title, before: before)
  render-fn(data, mk)
}

#let render-section-education(data, mk, before: 24pt) = _dispatch-section(
  data, [Education],
  _any-clusters-visible, render-education, mk, before: before,
)
#let render-section-appointments(data, mk, before: 24pt) = _dispatch-section(
  data, [Professional Appointments],
  _any-appointments-visible, render-appointments, mk, before: before,
)
#let render-section-publications(data, mk, before: 24pt) = _dispatch-section(
  data, [Scholarly Publications],
  _any-subsection-entries-visible, render-publications, mk, before: before,
)
#let render-section-presentations(data, mk, before: 24pt) = _dispatch-section(
  data, [Presentations],
  _any-subsection-entries-visible, render-presentations, mk, before: before,
)
#let render-section-research-support(data, mk, before: 24pt) = _dispatch-section(
  data, [Research Support],
  _any-research-support-visible, render-research-support, mk, before: before,
)
#let render-section-service(data, mk, before: 24pt) = _dispatch-section(
  data, [Professional Service],
  _any-service-visible, render-service, mk, before: before,
)
#let render-section-teaching(data, mk, before: 24pt) = _dispatch-section(
  data, [Teaching Experience],
  _any-teaching-visible, render-teaching, mk, before: before,
)
#let render-section-honors(data, mk, before: 24pt) = _dispatch-section(
  data, [Honors #emph[&] Awards],
  _any-flat-visible, render-honors, mk, before: before,
)
#let render-section-mentees(data, mk, before: 24pt) = _dispatch-section(
  data, [Mentees],
  _any-flat-visible, render-mentees, mk, before: before,
)
